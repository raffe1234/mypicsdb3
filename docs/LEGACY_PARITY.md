...
## Recommended roadmap after 0.8.25
...
9. Later — light video metadata, optional explicit map/reverse-geocoding provider action, optional generic provider API, rebuild catalogue while preserving sources; optional archive creation may reuse the 0.8.17 export engine if there is a real user need.
10. Later/high-risk — legacy import, duplicate reporting, sidecar-only refresh, mixed picture/video/music state machine.
