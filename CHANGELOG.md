...
- Maintenance: align location documentation with explicit 0.8.32 background bulk enrichment, bring the modernization milestones up to date and record scan-status presentation as a separate follow-up. Defer Open map, additional video metadata, XMP sidecars, ZIP/archive export and legacy database import until requested by a user.

- Add a code-level metadata extractor revision to the schema-9 `metadata_index_hash`. Parser/VFS fixes that can change normalized EXIF/XMP/IPTC/GPS results can now deliberately invalidate otherwise-unchanged catalogue rows even when user settings and mappings are identical. This closes the stale-index gap exposed by diagnostics where Fresh extraction found camera/GPS metadata but Indexed values remained empty under a matching old fingerprint; the next scan or **Refresh all picture metadata** re-reads those rows with the current extractor.
